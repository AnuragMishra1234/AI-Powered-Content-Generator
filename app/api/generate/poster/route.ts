...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!title) {
      return NextResponse.json(
        errorResponse('Please provide a poster title'),
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      successResponse('Poster generated successfully', {
        output,
    ...
    return NextResponse.json(
      errorResponse('Error generating poster', error.message),
      { status: 500 }
    );
  }
}
